...
/// These are set from TRUSTED_ADMIN_KEYS peer IDs or discovered dynamically
...
pub struct VersionTracker {
    ...
    /// Latest verified upgrade announcement from trusted admin
...
impl VersionTracker {
    ...
    /// Record a verified upgrade announcement from trusted admin
    pub fn record_upgrade(&self, upgrade: UpgradeAnnouncement) {
        // Only accept if signature verifies AND from trusted admin
        if !upgrade.verify() {
        ...
        if should_update {
            tracing::info!(
                "📦 VERIFIED upgrade announcement: v{} from admin {}...",
                upgrade.version,
                &upgrade.admin_pubkey[..16]
            );
...
pub struct BlockMessage {
    ...
    /// Ed25519 signature over SHA256(height || prev_state_root || challenge_hash || new_state_root || total_supply)
    #[serde(default)]
...
/// Signed upgrade announcement - broadcast by trusted admins
/// Only messages signed by keys in TRUSTED_ADMIN_KEYS are accepted
#[derive(Clone, Debug, Serialize, Deserialize)]
...
impl UpgradeAnnouncement {
    ...
    pub fn verify(&self) -> bool {
        ...
        // Parse public key
        let pubkey_bytes: [u8; 32] = match hex::decode(&self.admin_pubkey) {
            Ok(bytes) if bytes.len() == 32 => bytes.try_into().unwrap(),
            _ => return false,
        };
        ...
        // Build message: version || timestamp || mandatory || checksums
...
pub struct SmithNodeNetwork {
    ...
    _upgrade_topic: IdentTopic,  // Used for receiving only (admins broadcast from external tool)
    ...
    pub async fn new_with_data_dir(
        ...
        let local_key = if let Some(dir) = data_dir {
            ...
            if key_path.exists() {
                ...
                tracing::info!("🔐 Generated new P2P identity, saved to {:?}", key_path);
                keypair
            }
        } else {
            // No data dir - generate ephemeral keypair
    ...
    async fn handle_upgrade_message(&mut self, data: &[u8]) {
        match serde_json::from_slice::<UpgradeAnnouncement>(data) {
            Ok(upgrade) => {
                ...
                tracing::info!(
                    "📦 Received VERIFIED upgrade announcement: v{} from admin {}...",
                    upgrade.version,
                    &upgrade.admin_pubkey[..16]
                );
