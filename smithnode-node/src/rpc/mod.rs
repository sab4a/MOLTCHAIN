...
impl RateLimiter {
    ...
    pub async fn check(&self, operation: &str, key: &str, limit: usize) -> Result<(), String> {
        ...
        if entry.check_and_increment(limit) {
            // Periodically prune stale entries (every 100th check)
            if entries.len() > 1000 {
                let now = std::time::Instant::now();
                entries.retain(|_, v| {
                    now.duration_since(v.window_start) < Duration::from_secs(RATE_LIMIT_WINDOW_SECS * 5)
                });
            }
            Ok(())
        } else {
...
pub struct PuzzleResponse {
    ...
    // NOTE: expected_answer_hash is intentionally NOT exposed — that's the secret
}
...
pub struct UpdateCheckResponse {
    ...
    /// Verified download URL from trusted admin (if available)
    ...
    /// Whether the upgrade was verified by trusted admin signature
    pub verified: bool,
}
...
fn check_rate_limit(validator_pubkey: &str) -> Result<(), String> {
    ...
    let last_time = limiter.read().unwrap().get(validator_pubkey).copied();
    ...
    limiter.write().unwrap().insert(validator_pubkey.to_string(), now);
    Ok(())
}
...
impl SmithNodeRpcApiServer for SmithNodeRpcServerImpl {
    ...
    async fn register_validator(&self, req: RegisterValidatorRequest) -> RpcResult<SubmitProofResponse> {
        // No per-key rate limit needed: duplicate keys are rejected by state ("already registered"),
        // invalid keys are rejected cheaply, and general RPC rate limiting handles flood protection.
    ...
    async fn check_update(&self) -> RpcResult<UpdateCheckResponse> {
        ...
        // Get verified upgrade from trusted admin (if any)
