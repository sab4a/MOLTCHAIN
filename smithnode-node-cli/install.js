...
const SMITHNODE_DIR = join(homedir(), '.smithnode');
...
╚══════════════════════════════════════════════════════════════╝
    ...
    try {
      ...
      if (!existsSync(SMITHNODE_DIR)) {
        mkdirSync(SMITHNODE_DIR, { recursive: true });
      }
      console.log(colors.green(`   ✓ Created ${SMITHNODE_DIR}`));

      const sourceDir = join(SMITHNODE_DIR, 'source');
      ...
      const bootstrapPath = join(SMITHNODE_DIR, 'bootstrap.sh');
...
# Copy binary
cp target/release/smithnode ${SMITHNODE_DIR}/smithnode
...
`);
      ...
      const configPath = join(SMITHNODE_DIR, 'config.json');
      const defaultConfig = {
        rpc_port: 26658,
        p2p_port: 26656,
        celestia_rpc: null, // Optional: Celestia DA integration
        data_dir: join(SMITHNODE_DIR, 'data'),
        log_level: 'info',
      };
      ...
      const keyPath = join(SMITHNODE_DIR, 'validator-key.json');
...
program
  ...
  .action(async (options) => {
    ...
    const binaryPath = join(SMITHNODE_DIR, 'smithnode');
